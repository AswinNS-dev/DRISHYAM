      ...
      <div className="flex-1 p-4 sm:p-6 overflow-y-auto space-y-6 max-w-6xl mx-auto w-full">
        ...
        {telemetry && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
            <div className="p-4 rounded-xl bg-slate-950/70 border border-slate-800 shadow-md">
              ...
              <div className="text-lg md:text-xl font-bold text-white mt-1.5 font-mono">
                {telemetry.database || "PostgreSQL Core"}
              </div>
              <div className="text-xs font-mono text-emerald-400 mt-1 text-glow-emerald">Status: Operational</div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-950/70 border border-slate-800 shadow-md">
              ...
              <div className="text-2xl md:text-3xl font-black text-white mt-1.5 font-mono text-glow-white">
                {(telemetry.total_persons || 0) + (telemetry.total_vehicles || 0) + (telemetry.total_phones || 0)}
              </div>
              <div className="text-xs font-mono text-slate-400 mt-1">Persons, Phones & Plates</div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-950/70 border border-slate-800 shadow-md">
              ...
              <div className="text-xs font-mono text-slate-400 mt-1">Corroborated Linkages</div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-950/70 border border-slate-800 shadow-md">
              ...
              <div className="text-xs font-mono text-emerald-400 mt-1 text-glow-emerald">100% Tamper Evident</div>
            </div>
          </div>
        )}
            ...
            <table className="investigation-table text-sm w-full">
              <thead>
                <tr className="bg-slate-950/90 border-b border-slate-800 text-slate-400 font-mono text-xs uppercase tracking-wider font-bold">
                  ...
                  <th className="py-3 px-4 text-left">Clearance Role</th>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="font-bold text-white text-sm md:text-base py-3 px-4">{u.full_name}</td>
                    <td className="font-mono text-xs text-slate-300 py-3 px-4">{u.email}</td>
                    <td className="py-3 px-4">
                      <select
                        value={u.role}
                        onChange={(e) => handleRoleChange(u.id, e.target.value)}
                        className="bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs md:text-sm font-mono text-slate-200 outline-none cursor-pointer"
                      >
                    ...
                    <td className="py-3 px-4">
                      <span className="badge badge-verified text-xs font-mono font-bold text-glow-emerald">ACTIVE</span>
                    </td>
                    <td className="text-xs md:text-sm text-slate-300 leading-relaxed py-3 px-4">
                      {u.role === "admin"
        ...
        <div className="p-5 bg-slate-900/95 border border-slate-800/90 rounded-2xl shadow-xl space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800">
            ...
            <span className="badge badge-low text-xs font-mono font-bold">LOG TAMPER-SEALED</span>
          </div>
            ...
            <table className="investigation-table text-sm w-full">
              <thead>
                <tr className="bg-slate-950/90 border-b border-slate-800 text-slate-400 font-mono text-xs uppercase tracking-wider font-bold">
                  ...
                  <th className="py-3 px-4 text-left">Security Action</th>
                  <th className="py-3 px-4 text-left">Operator</th>
                  <th className="py-3 px-4 text-left">Detail & Record Target</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/60">
                {auditLogs.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="text-center py-6 text-slate-400 font-mono text-sm">
                  ...
                  auditLogs.map((log, idx) => (
                    <tr key={idx} className="hover:bg-slate-800/40 transition-colors">
                      <td className="font-mono text-xs text-slate-400 py-3 px-4">
                        {log.timestamp ? new Date(log.timestamp).toLocaleString() : "2026-09-05"} IST
                      </td>
                      <td className="py-3 px-4">
                        <span className="badge badge-low text-xs font-mono font-bold">{log.action || "AUDIT_EVENT"}</span>
                      </td>
                      <td className="font-bold text-white text-sm py-3 px-4">{log.user || "System Officer"}</td>
                      <td className="text-xs md:text-sm text-slate-200 leading-relaxed py-3 px-4">{log.details || log.target || "Routine verified access"}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        <div className="cmd-palette-backdrop" onClick={() => setShowUserModal(false)}>
          <div className="cmd-palette-modal max-w-md p-6 bg-slate-950 border border-slate-800 rounded-2xl shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              ...
              <button onClick={() => setShowUserModal(false)} className="text-sm font-mono text-slate-400 hover:text-white">
                ✕
              </button>
            </div>

            <form onSubmit={handleCreateUser} className="py-4 space-y-3.5 text-sm">
              <div>
                <label className="block text-xs font-mono uppercase text-slate-400 mb-1 font-bold">Full Legal Name</label>
                <input
                  required
              ...
              <div>
                <label className="block text-xs font-mono uppercase text-slate-400 mb-1 font-bold">Official Department Email</label>
                <input
                  type="email"
                  required
              ...
              <div>
                <label className="block text-xs font-mono uppercase text-slate-400 mb-1 font-bold">Initial Password</label>
                <input
                  type="password"
                  required
                  placeholder="••••••••"
                  ...
                  className="w-full bg-slate-900 border border-slate-800 rounded-lg p-2.5 text-sm text-slate-200 outline-none font-mono"
                />
              </div>

              <div>
                <label className="block text-xs font-mono uppercase text-slate-400 mb-1 font-bold">Clearance Level Assignment</label>
                <select
                  value={role}
              ...
              <div className="flex justify-end gap-2.5 pt-3 border-t border-slate-800">
                <button type="button" onClick={() => setShowUserModal(false)} className="px-4 py-2 rounded-lg bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 font-mono text-xs md:text-sm">
                  Cancel
                </button>
                <button type="submit" disabled={submitting} className="px-5 py-2 rounded-lg bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold text-xs md:text-sm shadow-md shadow-sky-500/20">
