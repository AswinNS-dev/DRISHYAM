    ...
    <div className="flex flex-col h-full bg-[#020617]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5 p-4 border-b border-slate-800/90 bg-slate-900/60 shrink-0">
        <div className="p-4 bg-slate-950/70 border border-slate-800 rounded-xl shadow-md">
          <div className="text-xs font-bold font-mono text-slate-400 uppercase tracking-wider">Sealed Forensic Assets</div>
          <div className="text-2xl md:text-3xl font-black font-mono text-white text-glow-white mt-1">{totalExhibits}</div>
        </div>
        <div className="p-4 bg-slate-950/70 border border-slate-800 rounded-xl shadow-md">
          <div className="text-xs font-bold font-mono text-slate-400 uppercase tracking-wider">Session Verifications</div>
          <div className="text-2xl md:text-3xl font-black font-mono text-emerald-400 text-glow-emerald mt-1">{verifiedCount} passed</div>
        </div>
        <div className="p-4 bg-slate-950/70 border border-slate-800 rounded-xl shadow-md">
          <div className="text-xs font-bold font-mono text-slate-400 uppercase tracking-wider">Hashing Algorithm</div>
          <div className="text-lg md:text-xl font-black font-mono text-sky-400 text-glow-sky mt-1.5">SHA-256 Engine</div>
        </div>
        <div className="p-4 bg-slate-950/70 border border-slate-800 rounded-xl shadow-md">
          <div className="text-xs font-bold font-mono text-slate-400 uppercase tracking-wider">Ledger Architecture</div>
          <div className="text-lg md:text-xl font-black font-mono text-purple-400 text-glow-purple mt-1.5">Chained Store</div>
        </div>
      </div>

      {/* ── Accurate Forensic Disclosure Notice ── */}
      <div className="mx-4 mt-3 p-3.5 bg-sky-950/40 border border-sky-500/40 rounded-xl text-sm text-slate-300 flex items-center gap-3 shadow-md shrink-0">
        <Lock size={18} className="text-sky-400 shrink-0" />
        <span className="leading-relaxed">
          <strong className="text-sky-300 font-bold text-glow-sky">CRYPTOGRAPHIC SEAL GUARANTEE:</strong> SHA-256 verification confirms that seized digital records have not been altered or tampered with since acquisition. It provides mathematical proof of bit-level custody integrity.
        </span>
      </div>
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-3.5">
        {loading ? (
          ...
          <div className="p-12 text-center text-sm text-slate-400 bg-slate-900/95 border border-slate-800 rounded-xl shadow-xl">
            <Fingerprint size={40} className="mx-auto mb-3 opacity-30 text-slate-500" />
            <div className="text-base font-bold text-slate-200 uppercase tracking-wider">No integrity records found</div>
            <p className="mt-1 text-slate-400 text-xs">No items match the current search criteria.</p>
          </div>
        ) : (
          <div className="space-y-3">
                ...
                >
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <div className="flex items-start gap-3.5 flex-1 min-w-0">
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center bg-slate-950 border border-slate-800 text-sky-400 shrink-0 mt-0.5 shadow-md">
                        <Fingerprint size={22} />
                      </div>

                      <div className="space-y-1.5 min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="text-base font-bold text-white hover:text-sky-300 transition-colors">
                          ...
                          {ev.source_record_id && (
                            <span className="text-xs font-mono text-sky-400 font-semibold text-glow-sky">
                    ...
                    <div className="lg:w-96 shrink-0 flex flex-col justify-between border-t lg:border-t-0 lg:border-l border-slate-800/80 pt-3 lg:pt-0 lg:pl-4 space-y-2.5">
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between text-xs font-mono">
                          ...
                          >
                            ...
                            <span className="text-xs font-semibold">{isCopied ? "Copied" : "Copy Digest"}</span>
                          </button>
                        </div>

                        <div className="p-2.5 rounded-lg bg-slate-950 border border-slate-800 font-mono text-xs text-sky-300 break-all select-all leading-tight shadow-inner">
                      ...
                      <div className="flex items-center justify-between gap-2 pt-1">
                        <div>
                          {verified ? (
                            <span className="badge badge-verified text-xs font-mono font-bold flex items-center gap-1.5 text-glow-emerald">
        ...
        <div className="cmd-palette-backdrop" onClick={() => setVerificationModal(null)}>
          <div className="cmd-palette-modal max-w-lg p-6 bg-slate-950 border border-slate-800 rounded-2xl shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              ...
              <button onClick={() => setVerificationModal(null)} className="text-sm font-mono text-slate-400 hover:text-white">
                ✕
              </button>
            </div>
            ...
            <div className="py-4 space-y-3.5 text-sm">
              <div className="p-3.5 rounded-xl bg-slate-900 border border-slate-800 space-y-1.5">
                <div className="text-xs font-mono uppercase text-slate-400">Exhibit Subject</div>
                <div className="font-bold text-white text-base">{verificationModal.exhibit.title}</div>
                <div className="text-xs font-mono text-slate-400">ID: {verificationModal.exhibit.id}</div>
              </div>

              <div className="space-y-1.5">
                <div className="text-xs font-mono text-slate-400 uppercase font-semibold">VERIFIED SHA-256 DIGEST</div>
                <div className="p-2.5 rounded-lg bg-slate-900 font-mono text-xs text-emerald-400 break-all border border-slate-800 shadow-inner">
                  {verificationModal.result.calculated_hash}
                </div>
              </div>

              <div className="grid grid-cols-2 gap-2.5 text-xs font-mono text-slate-300 pt-1">
              ...
              <div className="p-3.5 rounded-xl bg-emerald-950/40 border border-emerald-800/50 text-xs md:text-sm text-emerald-300 leading-relaxed font-mono">
            ...
            <div className="flex justify-end pt-3 border-t border-slate-800">
              <button onClick={() => setVerificationModal(null)} className="px-4 py-2 rounded-lg bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold text-sm shadow-md shadow-sky-500/20">
                Acknowledge & Close
              </button>
            </div>
          </div>
